...
- **详情**: 详见 CHANGELOG.md + user-manual.md §15.3

### TASK-P0-2: 选题雷达 / 爆款发现 ✅
- **完成**: 2026-06-16
- **Commit**: `977fcaf`
- **测试**: 33/33 passed
- **验证**: 真 LLM AI 改写 + 网络 fail mock 兜底
- **详情**: CHANGELOG.md + user-manual.md §15.1
- **未做**: 前端 UI (Sources 热榜 tab) — API 已可手动调用

### TASK-P0-3: 一稿多发 / 平台改写引擎 ✅
- **完成**: 2026-06-16
- **Commit**: `a4eaf45`
- **测试**: 16/16 passed (4 平台并发 + 模板 + 落库)
- **详情**: CHANGELOG.md + user-manual.md §15.2
- **未做**: 前端 UI (AI 中心"一稿多发" tab) — API 已可手动调用

### TASK-P0-10: 视频号 (WeChat Channels) Adapter ✅ (快赢)
- **完成**: 2026-06-16
- **测试**: 12/12 passed
- **策略**: 注册 + 框架完成,核心 upload 抛 `WeixinChannelsNotConfiguredError`
- **详情**: CHANGELOG.md
- **下一 PR**: 集成 `tencent_uploader` + 扫码 cookie 流程

---

## 累计战果(2026-06-15 至 2026-06-16)

| 指标 | 数字 |
|------|------|
| 提交 | 7 (P0-1, P0-2, P0-3, P0-10, 3× 文档) |
| 后端测试 | 111/111 ✅ (50 wechat + 33 hot + 16 adapt + 12 weixin_channels) |
| 新增端点 | 7 (`/api/hot` 4 个, `/api/ai/adapt` 2 个, `/api/platforms/publish-article-now` 1 个) |
| 平台覆盖 | 5 → 6 (新增视频号) |
| 新增前端 UI | "发布到公众号" 按钮 + dialog |
| 文档 | user-manual.md §15 (3 节) + CHANGELOG (4 归档) + TASKS + roadmap |
...
下一阶段 P1 (去 AI 味 / 数据回流 / GEO 优化) 留待后续会话。
